...
global_map_lock = threading.Lock()

def add_to_global_map(aruco_id, points, poses):
    with global_map_lock:
        global_map[aruco_id] = {"points": points.copy(), "pose": poses.copy()}
        global_map_show[aruco_id] = True  # 預設顯示

def remove_from_global_map(aruco_id):
    with global_map_lock:
